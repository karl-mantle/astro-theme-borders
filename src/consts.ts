import OGImage from './assets/cube.jpg';

// site
export const SITE_TITLE = 'Astro Blog';
export const SITE_TITLE_SEPARATOR = '-';
export const SITE_DESCRIPTION = 'Welcome to my website!';
export const SITE_SHORT_TITLE = 'A blog';
export const SITE_FOOTER_TEXT = '© MMXXIV - MMXXV - all rights reserved.';
export const SITE_LANGUAGE = 'en-US';
export const DATE_FORMAT = 'j F Y';
export const TIME_FORMAT = 'g:i A';

// content settings
export const POSTS_PER_PAGE = 9;
export const PERMALINK_POSTS = 'blog/';
export const PERMALINK_POSTS_SINGLE = 'blog/detail/';
export const PERMALINK_POSTS_CATEGORY = 'blog/category/';
export const PERMALINK_POSTS_TAG = 'blog/tag/';

export const CUSTOM_PER_PAGE = 9;
export const PERMALINK_CUSTOM = 'custom/';
export const PERMALINK_CUSTOM_SINGLE = 'custom/detail/';
export const PERMALINK_CUSTOM_CATEGORY = 'custom/category/';
export const PERMALINK_CUSTOM_TAG = 'custom/tag/';

// images
export const SITE_OG_IMAGE = OGImage;
export const SITE_LOGO_SVG_SQUARE = './assets/logo/svg/Logo-1-1.svg';

// seo/schema

// theme
export const THEME_CONTAINER_DESKTOP = 'w-full max-w-screen-xl mx-auto';
export const THEME_CONTAINER_MOBILE = 'px-8 xl:px-0';